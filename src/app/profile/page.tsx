...
export default function ProfilePage() {
  ...
  const [groupBalances, setGroupBalances] = useState<{
    [groupId: string]: { amount: number; type: string };
  }>({});
        ...
        if (res.data.success && res.data.expenses) {
          ...
          res.data.expenses.forEach((expense: any) => {
            if (expense.balanceForCurrentUser) {
              ...
              total +=
                expense.balanceForCurrentUser.type === "lent"
                  ? amount
                  : -amount;
            }
          });
              ...
              {groups.length > 0 ? (
                ...
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <p className="text-gray-500">
                    You haven&apos;t created any expense groups yet
                  </p>
                  <p className="text-gray-400 text-sm mt-1">
                    Groups make it easy to split and track shared expenses
                  </p>
                </div>
              )}
            </div>
                ...
                <div>
                  ...
                  <div className="flex items-center space-x-2 mb-4">
                    <input
                      type="text"
                      value={searchQuery}
                      onChange={(e) => setSearchQuery(e.target.value)} // Only update state, don't search yet
                      className="block w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors placeholder-gray-600 text-gray-600"
